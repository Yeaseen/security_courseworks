...
def optimize_model():
    ...
    non_final_next_states = torch.cat([s for s in batch.next_state if s is not None]).to(torch.float32)
    state_batch = torch.cat(batch.state).to(torch.float32)
    action_batch = torch.cat(batch.action).to(torch.int64)  # Ensure integer dtype for indexing
