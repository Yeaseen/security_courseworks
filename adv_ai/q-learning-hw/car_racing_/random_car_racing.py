...
for episode in tqdm(range(num_episodes), desc="Training Random Policy - CarRacing"):
    state, _ = env.reset()
    total_reward = 0
    done = False
...
plt.title("Random Policy Learning Curve - Car Racing")
plt.xlabel("Episode")
plt.ylabel("Average Reward")
plt.plot(reward_moving_average, label='Random Policy Moving Average Reward')
plt.legend()
plt.tight_layout()
plt.savefig("random_policy_car_racing.png")
