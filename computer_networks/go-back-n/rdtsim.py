...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.base = 0
        self.next_seqnum = 0
        self.message_list = []  # list to store messages when the window is full
        self.currIndex = 0
    ...
    def output(self, message):
        checksum = calculate_checksum(self.next_seqnum, 0, message.data)
        packet = Pkt(self.next_seqnum, 0, checksum, message.data)
        self.message_list.append(packet)  # Store for potential retransmission
        self.next_seqnum = (self.next_seqnum +1) % self.seqnum_limit
        self.send_next_pkt()


    def send_next_pkt(self):
        while (self.currIndex < self.base + self.window_size) and (self.currIndex < (len(self.message_list)-1)):
            to_layer3(self, self.message_list[self.currIndex])
            if(self.base == self.currIndex):
                    start_timer(self, 18)
            self.currIndex += 1
    ...
    def input(self, packet):
        ...
        for i in range(self.base, self.base + self.window_size):
            if (i < len(self.message_list)):
                if self.message_list[i].seqnum == packet.acknum:
                    self.base = i + 1
                    break
    ...
    def timer_interrupt(self):
        for i in range(self.base, self.base + self.window_size):
            if (i < len(self.message_list)):
                to_layer3(self, self.message_list[i])  # Retransmit the packet
        start_timer(self,18)
